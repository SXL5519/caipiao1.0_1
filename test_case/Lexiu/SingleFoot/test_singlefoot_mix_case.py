...
class PaintBall_Single_mix_case(MyTest):
    """竞足单关，混合玩法"""
    def test_Play_mix_choosenumber_case(self):
        ...
        hb.Play_f()####点击玩法
        hb.Play_mix()###点击混合玩法
        aa=hb1.Paintball_single_mix()##点击混合玩法显示的赛事结果
        if aa == 1:
            hb1.Paintball_single_mixs(4)####随机点击n场赛事
    ...
    def test_Play_mix_choosenumber_eight_case(self):
        ...
        hb.Play_f()  ####点击玩法
        hb.Play_mix()  ###点击混合玩法
        aa=hb1.Paintball_single_mix_nus_X(8)####随机点击8场赛事
        if aa>0:
            hb.confirm_match()###点击已选N场比赛
            hc.submit_order_to_station_owner_button()####点击提交给站主
            #hl.new_user_login_tab()  # 点击新登录
    ...
    def test_Play_mix_choosenumber_nine_case(self):
        ...
        hb.Play_f()  ####点击玩法
        hb.Play_mix()  ###点击混合玩法
        aa = hb1.Paintball_single_mix_nus_X(9)  ####随机点击9场赛事
        if aa>0:
            hb.confirm_match()###点击已选N场比赛
            hc.submit_order_to_station_owner_button()####点击提交给站主
            #hl.new_user_login_tab()  # 点击新登录
    ...
    def test_Play_mix_emptying(self):
        ...
        if aa>0:
            hb1.Emptying_pbs()
            hb.confirm_match()  ###点击已选N场比赛
            hb1.Paintball_single_mix_nus_X(5)  ###随机点击5场比赛
            hb.confirm_match()  ###点击已选N场比赛
            hc.submit_order_to_station_owner_button()  ####点击提交给站主
            # hl.new_user_login_tab()  # 点击新登录
            hl.login_lexiu()  # 输入账号，密码
