...
class gxEleChooseFiveHandCase(MyTest):
    ...
    def test_option16_hand_choose3_case(self):
        ...
        efcn.countdown_text()  # 检查“距离第xxx期截止：”文案是否存在
        efcn.spread_mode_button()  ##选择展开玩法
        efcn.mode_choose(22, 15)  # 广西11选5乐选五
    ...
    def test_play_instruction_case(self):
        ...
        hp = HomePage_lexiu(self.driver)
        tD = There_D_choosenumber_lexiu(self.driver)
        ulcn = UnionLottoChooseNumber_lexiu(self.driver)
        efcn = ElevenFiveChooseNumber_lexiu(self.driver)
        hp.open()
        hp.Moveable_float_close()
    ...
    def test_orderdetails_page_continue_buy_case(self):
        ...
        num2 = od.Ele_five_betting_nu()  # 获取订单详情页的号码
        self.assertEqual(num1, num2)
    def test_one_lottery_one_times_case(self):
        '''单式单注，进入智能追号页面，默认追期，提交订单'''
        hp = HomePage_lexiu(self.driver)
        ulcn = UnionLottoChooseNumber_lexiu(self.driver)
        efcn = ElevenFiveChooseNumber_lexiu(self.driver)
        efcl = EleChooseFiveConfirmLottery_lexiu(self.driver)
    ...
    def test_double_lottery_double_times_case(self):
        '''复式，进入智能追号页面，默认追期，提交订单'''
        hp = HomePage_lexiu(self.driver)
        ulcn = UnionLottoChooseNumber_lexiu(self.driver)
        efcn = ElevenFiveChooseNumber_lexiu(self.driver)
        efcl = EleChooseFiveConfirmLottery_lexiu(self.driver)
