...
from element_operate.baofoo_payment import BaofooPayment_leyou
from element_operate.choose_bank import ChooseBank_leyou
from element_operate.confirm_pay import ConfirmPay_leyou
...
from element_operate.payment_mode import PaymetMode_leyou
...
class LessPayment_case(MyTest):
    ...
    def test_less_payment_shortfall_case(self):
        ...
        l.login_nomoney_leyou()#点击登录
        cl.submit_order_to_station_owner_button()  # 点击“提交订单给站主”按钮
        cl.confirm_and_pay_button()  # 点击“确认并支付”按钮
        pm = PaymetMode_leyou(self.driver)
    ...
    def test_recharge_case(self):
        """验证充值流程"""
        hp = HomePage_leyou(self.driver)
        hb=MyTicket_leyou(self.driver)
        hc=PaymetMode_leyou(self.driver)
        hd=BaofooPayment_leyou(self.driver)
        he=ChooseBank_leyou(self.driver)
        hf=ConfirmPay_leyou(self.driver)
