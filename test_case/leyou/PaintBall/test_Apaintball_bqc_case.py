...
class PaintBall_bqc(MyTest):
    ...
    def test_Play_bqc_choosenumber_too_case(self):
        ...
        hb.Play_f()  ###点击玩法
        hb.Play_bqc()  ##点击比分
        aa = hb.Football_bqc_nus_too()  ####点击2场赛事
        if aa == 1:
    ...
    def test_Play_bqc_N_del(self):
        ...
        if aa > 0:
            hb.confirm_match()  ###点击已选N场比赛
            a = hc1.BF_nn()  ###读取删除前有多少个X按钮
            hc1.Del_n(1)  ##点击第1场赛事的X按钮
            a1 = hc1.BF_nn()  ###读取删除后有多少个X按钮
    ...
    def test_Play_bqc_Btn_twenty(self):
        """验证20倍按钮功能，支付流程"""
        ha = HomePage_leyou(self.driver)
        hb = PaintBallChooseNumber_leyou(self.driver)
        hc = ConfirmLottery_leyou(self.driver)
    ...
    def optimize_award_case(self):
        ...
        pbcn.Play_f()  ###点击玩法
        pbcn.Play_bqc()  ##点击胜平负
        aa = pbcn.Football_bqc_X(4)  ###随机点击4场比赛
        if aa > 0:
