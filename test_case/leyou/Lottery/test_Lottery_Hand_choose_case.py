...
from element_operate.BigLottery.BigLottery_choosenumber import BigLotteryChooseNum_leyou
...
from element_operate.BigLottery.BigLottery_confirm_lottery import BigLotteryConfirmLottery_leyou
...
#################mj20171208
    ...
    def test_Lottery_multiphase_machine_input_case(self):
        ...
        text = sos.submit_order_success()
        self.assertEqual("订单提交成功", text, "提交订单失败")
    def test_Lottery_multiphase_machine_add_case(self):
        """选号页面，右上方隐藏的多期机选，点击+，提交订单"""
        ###点击进入大乐透进入大乐透选号页面###
        hp = HomePage_leyou(self.driver)
        hp.open()
        ######判断是否出现浮层弹框，如果出现浮层点击X，然后执行下一步操作
        ...
        self.assertIn(str(text2+1),text3,'多期机选有误')
        ulcn.Many_pause_confirm()#点击多期机选页的【提交订单】
        cl.confirm_and_pay_button()  # 点击“确认并支付”按钮
        '''断言验证提交订单成功'''
        sos = SubmitOrderSuccess_leyou(self.driver)
        ...
        text = sos.submit_order_success()
        self.assertEqual("订单提交成功", text, "提交订单失败")
    def test_Lottery_multiphase_machine_sub_case(self):
        """选号页面，右上方隐藏的多期机选，点击-，提交订单"""
        ###点击进入大乐透进入大乐透选号页面###
        hp = HomePage_leyou(self.driver)
        hp.open()
