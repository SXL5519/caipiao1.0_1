...
class Login_loc():
    ######新用户登录定位##########
    new_user_login_tab_loc = (By.ID,"login_tab_v2")
    #####用户名输入框定位#####
...
class HomePage_loc():
    ...
    gd_11_5_link_loc = (By.CLASS_NAME,'logo-11x5gd')#广东11选5定位
    paintball_link_loc = (By.CLASS_NAME,"logo-jczq")##竞彩足球定位
    single_foot_link_loc = (By.CLASS_NAME,"logo-jzdg")##竞足单关定位
    gx_11_5_link_loc = (By.CLASS_NAME,'logo-11x5gx')#广西11选5定位
    ...
    cqssc_link_loc = (By.CLASS_NAME,'logo-ssccq')  ##重庆时时彩元素定位
    victory_or_defeat_link_loc = (By.CLASS_NAME,"logo-sfc")#胜负14场
    activities_loc = (By.CSS_SELECTOR,"section[id*='mi_act_1'] div[class*='ui-avc']")#活动专区定位
    more_lottery_link_loc = (By.CLASS_NAME,"logo-no")#更多彩种定位
    rank_three_link_loc = (By.CLASS_NAME,"logo-pls")#排列3定位
    rank_five_link_loc = (By.CSS_SELECTOR,'i[style*="66AD8DA7E5D24C64B6F9670057D97EB7"]')#排列5元素定位
    colorful_star_link_loc = (By.CLASS_NAME,"logo-qxc")#七彩星定位
    jiangxi_express_three_link_loc = (By.CLASS_NAME,"logo-ksjx")#江西快三元素定位
    shandong_11x5_link_loc = (By.CSS_SELECTOR,'i[style*="A39BE5287DE443E48880D66253362B7A"]')#山东11选5元素定位
    ############我的彩票定位
...
##11选5页选号页元素定位
class EleChooseFiveChooseNum_loc():
    # 当前期定位
...
###大乐透投注确认页元素定位
class BigLotteryConfirmLottery_loc():
...
###11选5投注确认页元素定位
class EleChooseFiveConfirmLottery_loc():
...
class OderDetails_loc():
    ...
    betting_number=(By.CSS_SELECTOR,'i.off')###投注号码定位
...
##############################################竞彩足球
####################混合投注 选号页面定位
class PaintBallChooseNumber_loc():
...
class HaobcChooseNumber_loc():##选号页元素定位
...
class HaobcLotteryConfirm():##竞彩篮球投注确认页
    edit_event_loc=(By.ID,'jcleditmatch')#编辑赛事按钮定位

#######竞足单关元素定位-----mj20171116
class SingleFootChooseNumber_loc():
    ...
    more_pbs = (By.CLASS_NAME, 'jjc_table>tbody>tr:nth-child(1)')  ###展开更多里的玩法
...
class SingleFootLotteryConfirm_loc():
...
class CQSSC_ConfirmLottery_loc():
