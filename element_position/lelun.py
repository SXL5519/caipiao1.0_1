...
###首页定位器###############################################
class HomePage_lelun_loc():
    header_text_loc = (By.CLASS_NAME,"ui-cvertical")##首页顶部文本定位
    ###活动浮层弹窗定位
    ...
    gd_11_5_link_loc = (By.CSS_SELECTOR, "i[style*='img_key=3F0070F4F7314B01BCD3DFC670B369E5']")#广东11选5定位
    paintball_link_loc = (By.CLASS_NAME,"logo-jczq")##竞彩足球定位
    single_foot_link_loc = (By.CLASS_NAME,"logo-jzdg")##竞足单关定位
    gx_11_5_link_loc = (By.CSS_SELECTOR,"i[style*='6E32E6A6C92E4FDF8B60DE9B4F58543']")#广西11选5定位
    ...
    cqssc_link_loc = (By.CSS_SELECTOR, "i[style*='037F598F72F14E68AA538A0A530C5602']")  ##重庆时时彩元素定位
...
#############投注确认页元素定位##########
class ConfirmLottery_lelun_loc():
    ...
    delete_all_num_button_loc = (By.CSS_SELECTOR, '#ptjxbtns>.btn_white.b-flex.btnlnk>.i-delback')  # 清除列表按钮
...
###提交訂單成功頁元素定位###############################################
class SubmitOrderSuccess_lelun_loc():
    # 获取“提交订单成功”文本定位
...
##11选5页选号页元素定位
class EleChooseFiveChooseNum_lelun_loc():
    # 当前期定位
...
###大乐透投注确认页元素定位
class BigLotteryConfirmLottery_lelun_loc():
...
###11选5投注确认页元素定位
class EleChooseFiveConfirmLottery_lelun_loc():
...
###我的彩票页定位器###############################################
class MyTicket_lelun_loc():
    recharge = (By.CSS_SELECTOR, '.btn_recharge')  ##充值按钮
    setup_link_loc = (By.CLASS_NAME,"ui-avc.accmenuli")###设置链接定位##
...
##############################################竞彩足球
####################混合投注 选号页面定位
class PaintBallChooseNumber_lelun_loc():
...
class PaintBallConfirm_lelun_loc():
    add_event=(By.CSS_SELECTOR,'div[ontap="jczdgpage.jczdeal.retsel();"]')#####添加/编辑赛事
    add_event2=(By.CSS_SELECTOR,'div[ontap="jczpage.jczdeal.retsel();"]')##添加/编辑赛事
    def n_del(self,n):
...
class HaobcChooseNumber_lelun_loc():##选号页元素定位
...
class HaobcLotteryConfirm_lelun():##竞彩篮球投注确认页
    edit_event_loc=(By.ID,'jcleditmatch')#编辑赛事按钮定位

#######竞足单关元素定位-----mj20171116
class SingleFootChooseNumber_lelun_loc():
    ...
    more_pbs = (By.CSS_SELECTOR,'.jjc_table> tbody>tr:nth-child(1)')  ###展开更多里的玩法
...
class SingleFootLotteryConfirm_lelun_loc():
...
###重庆时时彩选号页元素定位
class CQSSC_ChooseNumber_lelun_loc():
...
class CQSSC_ConfirmLottery_lelun_loc():
...
###############################江西快三
class JXKS_ChooseNumber_lelun_loc():
    def all_play(self,n):
...
class RX9C_choosenumber_lelun_loc():
...
class RX9C_ConfirmLottery_lelun_loc():
