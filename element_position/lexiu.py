...
###首页定位器###############################################
class HomePage_lexiu_loc():
    header_text_loc = (By.CLASS_NAME,"backtitle")##首页顶部文本定位
    ###活动浮层弹窗定位
    Moveable_float_close_loc = (By.CSS_SELECTOR,'.i-plusdel.mi_ldialog_close')
    ######################彩种定位############################################
    UnionLotto_link_loc = (By.CLASS_NAME, "logo-ssq")#双色球定位
    Lottery_link_loc = (By.CLASS_NAME, "logo-dlt")#大乐透定位
    gd_11_5_link_loc = (By.CSS_SELECTOR, "i[style*='7D63BEFA23CF4F4A92B184F24635C30D']")#广东11选5定位
    paintball_link_loc = (By.CLASS_NAME,"logo-jczq")##竞彩足球定位
    single_foot_link_loc = (By.CLASS_NAME,"logo-jzdg")##竞足单关定位
    gx_11_5_link_loc = (By.CSS_SELECTOR,"i[style*='DA451F12D117494BBDD8FE2B4E3F6829']")#广西11选5定位
    ...
    cqssc_link_loc = (By.CSS_SELECTOR, "i[style*='0FD103DEDB9A424BBEFEFDDCC52B9FEE']")  ##重庆时时彩元素定位
    ...
    ############我的彩票定位
    my_lottery_ticket = (By.CLASS_NAME, "nav_icone")  ###首页我的彩票
    ...
    HomePage_link_loc = (By.CLASS_NAME,"nav_icona")
    home_page = (By.CLASS_NAME, "nav_icona")  ###首页
...
class UChooseNum_lexiu_loc():
    ...
    lottery_information_unionlotto=(By.CSS_SELECTOR,'li[ontap*="#?page=kjgg_list&lot_code=50"]')###开奖信息，双色球
...
#############投注确认页元素定位##########
class ConfirmLottery_lexiu_loc():
    confirm_num_page_text_loc = (By.PARTIAL_LINK_TEXT, "投注确认")  # 投注确认页，“投注确认”文本定位
    continue_choose_num_button_loc = (By.CSS_SELECTOR, "div[id*='contisel'] i[class*='i-plus")  # “继续选号”按钮定位
    ...
    unionlotto_History_buy=(By.CSS_SELECTOR,'#openCodes > ul > li:nth-child(1)')##历史开奖，双色球展示的最近开奖信息
...
###提交訂單成功頁元素定位###############################################
class SubmitOrderSuccess_lexiu_loc():
    # 获取“提交订单成功”文本定位
...
##11选5页选号页元素定位
class EleChooseFiveChooseNum_lexiu_loc():
    # 当前期定位
...
###大乐透投注确认页元素定位
class BigLotteryConfirmLottery_lexiu_loc():
...
###11选5投注确认页元素定位
class EleChooseFiveConfirmLottery_lexiu_loc():
    ...
    chase_ticket_button_loc = (By.ID, "issue_num_id")  # 选择追xx期定位
...
##############################################竞彩足球
####################混合投注 选号页面定位
class PaintBallChooseNumber_lexiu_loc():
...
class PaintBallConfirm_lexiu_loc():
    add_event=(By.CSS_SELECTOR,'div[ontap="jczpage.jczdeal.retsel();"]')#####添加/编辑赛事
    add_event2=(By.CSS_SELECTOR,'div[ontap="jczdgpage.jczdeal.retsel();"]')##添加/编辑赛事

    def n_del(self,n):
        n=(By.XPATH,'//*[@id="buyinfsel"]/dl[%d]/dd/ul/li/div[3]/i'%n)####清除一场赛事X定位
        return n
    ...
    nn = (By.CLASS_NAME, 'i-del')  ###所有的赛事X定位
...
class HaobcChooseNumber_lexiu_loc():##选号页元素定位
...
class HaobcLotteryConfirm_lexiu():##竞彩篮球投注确认页
    edit_event_loc=(By.ID,'jcleditmatch')#编辑赛事按钮定位

#######竞足单关元素定位-----mj20171116
class SingleFootChooseNumber_lexiu_loc():
    ...
    confirm_more_pbs = (By.CSS_SELECTOR, 'a[href="javascript:jczdgpage.jczdeal.confirmC();"]')  ###展开更多页面确认按钮
    ...
    more_pbs = (By.CLASS_NAME, 'jjc_table')  ###展开更多里的玩法
...
class SingleFootLotteryConfirm_lexiu_loc():
...
###重庆时时彩选号页元素定位
class CQSSC_ChooseNumber_lexiu_loc():
...
class CQSSC_ConfirmLottery_lexiu_loc():
...
###############################江西快三
class JXKS_ChooseNumber_lexiu_loc():
    def all_play(self,n):
...
class RX9C_choosenumber_lexiu_loc():
...
class RX9C_ConfirmLottery_lexiu_loc():
